...
export const EventsTable = () => {
  ...
  const handleEdit = (value: EventType) => {
    ...
    redirect(`/dashboard/school_event/${value.id}`);
  };
  ...
  const deleteEntity = async () => {
    try {
      if (IdDel !== -1) {
        await ApiService.deleteEvent(IdDel).then(() => fetchEntity());
      }
    } catch (error) {
      console.log(error);
    }
  ...
  const fetchEntity = async () => {
    try {
      ...
      const data = await ApiService.events(`${query}`);
    ...
    <div className="p-6 bg-white rounded-lg shadow-md">
      ...
      >
        ...
        <input
          ...
          onChange={(e) =>
            handleFilterChange("description__contains", e.target.value)
          }
        />
        <input
          type="text"
          ...
          className="mt-1 p-2 block w-full rounded-md shadow-sm focus:border-blue-500 focus:ring-blue-500"
          onChange={(e) =>
            handleFilterChange("title__contains", e.target.value)
          }
        />
      </div>
