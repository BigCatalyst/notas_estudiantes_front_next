...
import { SchoolYear } from "@/services/api/school_year";
...
const SchoolYearTable = () => {
  ...
  const handleEdit = (value: SchoolYear) => {
    // Lógica para editar
    console.log(value);
  ...
  const deleteEntity = async () => {
    try {
      if (IdDel !== -1) {
        await ApiService.deleteSchoolYear(IdDel).then(() => fetchEntity());
      }
    } catch (error) {
      console.log(error);
    }
  ...
  const fetchEntity = async () => {
    try {
      ...
      const data = await ApiService.schoolYears(`${query}`);
      ...
      >
        ...
        {/* <div className="mt-1 p-2 flex items-center justify-center w-full rounded-md shadow-sm focus:border-blue-500 focus:ring-blue-500 gap-7">
          ...
          <input
            ...
            type="date"
            onChange={(e) =>
              handleFilterChange("end_date__lte", e.target.value)
            }
          />
        </div> */}
      </div>
