...
import { Subject, SubjectGet } from "@/services/api/subjects";
...
export const LoteEditStudentNote = () => {
  ...
  const [subjects, setSubjects] = useState<SubjectGet[] | null>(null);
  ...
  useEffect(() => {
    (async () => {
      try {
        const res = await ApiService.studentsAll("");
  ...
  const fetchEntity = async () => {
    try {
      ...
      if (data) {
        ...
        let subData: SubjectGet[] = [];
        data.forEach((val) => {
          (async () => {
            const res = await ApiService.getSubject(String(val.subject));
            if (res) subData.push(res);
          })().then(() => {
    ...
    <div className="p-6 bg-white rounded-lg shadow-md">
      ...
      >
        <select
          ...
          value={idStudent}
          onChange={(e) => {
            handleFilterChange("student__id", e.target.value);
            setIdStudent(Number(e.target.value));

            const selectedStudentId = e.target.value;
            {/*
            const selectedSubject = students.find(
              (subject) => subject.id + "" == selectedStudentId
            );
  ...
  {!loading &&
    ...
    list.map((item, index) => {
      const subjectName =
        subjects?.find((s) => s.id === item.subject)?.name ?? "none";

      return (
        <tr key={`${item.subject}-${item.student}`} className="border-b border-b-gray-300">
          {/* Asignatura */}
